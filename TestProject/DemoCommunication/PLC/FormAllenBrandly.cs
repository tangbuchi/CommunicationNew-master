    ...
    public partial class FormAllenBrandly : Form
    {
        ...
        private void button1_Click( object sender, EventArgs e )
        {
            ...
            try
            {
                ...
                {
                    MessageBox.Show( Communication.StringResources.Language.ConnectedFailed + connect.ToMessageShowString( ) );
                }
            }
            catch (Exception ex)
            {
        ...
        {
            // MessageBox.Show( Communication.BasicFramework.SoftBasic.ByteToHexString( allenBradleyNet.BuildReadCommand( textBox3.Text ).Content, ' ' ) );
            // 读取byte变量
        ...
        {
            // bool写入
            //MessageBox.Show( Communication.BasicFramework.SoftBasic.ByteToHexString( allenBradleyNet.BuildWriteCommand( textBox8.Text, AllenBradleyHelper.CIP_Type_Bool, (bool.Parse( textBox7.Text ) ?
        ...
        private void button25_Click( object sender, EventArgs e )
        {
            ...
            //    OperateResult write = allenBradleyNet.Write( "Array", new short[] { 101, 102, 103, 104, 105, 106 } );
                ...
                {
                    //MessageBox.Show( Communication.BasicFramework.SoftBasic.ByteToHexString( allenBradleyNet.BuildReadCommand( new string[] { textBox6.Text }, new int[] { int.Parse(textBox9.Text) } ).Content , ' ') );
                    read = allenBradleyNet.Read( textBox6.Text, ushort.Parse( textBox9.Text ) );
                }
                else
                {
                    //MessageBox.Show( Communication.BasicFramework.SoftBasic.ByteToHexString( allenBradleyNet.BuildReadCommand( textBox6.Text.Split( ';' ) ).Content, ' ' ) );
        ...
        {
            OperateResult<byte[]> read = allenBradleyNet.ReadFromCoreServer( Communication.BasicFramework.SoftBasic.HexStringToBytes( textBox13.Text ) );
            if (read.IsSuccess)
            {
