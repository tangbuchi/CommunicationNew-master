...
namespace CommunicationDemo
{
    public partial class FormRegister : Form
    {
        ...
        private Communication.BasicFramework.SoftAuthorize softAuthorize = null;

        private void FormRegister_Load( object sender, EventArgs e )
        {
            softAuthorize = new Communication.BasicFramework.SoftAuthorize(Settings1.Default.UseAdmin);            // 实例化
            softAuthorize.ILogNet = new Communication.LogNet.LogNetSingle( "log.txt" );   // 日志
        ...
        {
            // 此处使用了组件支持的DES对称加密技术
            return Communication.BasicFramework.SoftSecurity.MD5Encrypt( origin, textBox1.Text );
        }
        ...
        private void button3_Click( object sender, EventArgs e )
        {
            ...
            {
                // 显示注册窗口
                using (Communication.BasicFramework.FormAuthorize form =
                    new Communication.BasicFramework.FormAuthorize(
                        softAuthorize,
                        "请联系XXX获取激活码",
                        AuthorizeEncrypted ))
                {
