...
namespace CommunicationDemo
{
    public partial class FormModbusAlien : Form
    {
        ...
        {
            InitializeComponent( );
            logNet = new Communication.LogNet.LogNetSingle( "modbus.txt" );
        }


        private ModbusTcpNet busTcpClient = null;
        private Communication.LogNet.ILogNet logNet = null;
        ...
        private void button25_Click( object sender, EventArgs e )
        {
            try
            {
                ...
                if (read.IsSuccess)
                {
                    textBox10.Text = "结果：" + Communication.BasicFramework.SoftBasic.ByteToHexString( read.Content );
                }
                else
                {
        ...
        private void button26_Click( object sender, EventArgs e )
        {
            try
            {
                ...
                if (read.IsSuccess)
                {
                    textBox11.Text = "结果：" + Communication.BasicFramework.SoftBasic.ByteToHexString( read.Content );
                }
                else
                {
                    MessageBox.Show( "读取失败：" + read.ToMessageShowString( ) );
