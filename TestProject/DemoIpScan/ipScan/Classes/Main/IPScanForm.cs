...
namespace ipScan.Classes.Main
{
    public partial class IPScanForm : Form
    {
        ...
        public static bool IPScanDone = false;
        ...
        public async void StartScan(string IPFirst, string IPLast)
        {
            ...
            if (!IsRunning)
            {
                ...
                try
                {
                    _firstIpAddress = IPTools.IPAddress2UInt32(IPFirst);
                }
                ...
                try
                {
                    _lastIpAddress = IPTools.IPAddress2UInt32(IPLast);
                }
                catch (FormatException ex)
                {
                    _logger.Error(ex);
                ...
                _logger.Info(string.Concat("Start searching from [", IPFirst, "] to [", IPLast, "]"));
