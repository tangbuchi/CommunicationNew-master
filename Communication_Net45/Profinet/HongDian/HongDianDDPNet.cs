...
namespace Communication.Profinet.HongDian
{
    /// <summary>
    ...
    /// </example>
    public class HongDianDDPNet : NetworkDeviceBase<S7Message, ReverseBytesTransform>
    {
        #region Constructor
        ...
        public HongDianDDPNet(HongDianDTUS siemens )
        {
        ...
        public HongDianDDPNet(HongDianDTUS siemens, string ipAddress )
        {
        ...
        private void Initialization(HongDianDTUS siemens, string ipAddress )
        {
            WordLength   = 2;
            IpAddress    = ipAddress;
            Port         = 102;
            ...
            switch (siemens)
            {
                case HongDianDTUS.S1200:    plcHead1[21] = 0; break;
                case HongDianDTUS.S300:     plcHead1[21] = 2; break;
                case HongDianDTUS.S400:     plcHead1[21] = 3; plcHead1[17] = 0x00; break;
                case HongDianDTUS.S1500:    plcHead1[21] = 0; break;
                case HongDianDTUS.S200Smart:
                    {
                        plcHead1 = plcHead1_200smart;
                        plcHead2 = plcHead2_200smart;
                        break;
                    }
                case HongDianDTUS.S200:
                    {
        ...
        public override OperateResult Write( string address, string value )
        {
            ...
            if (CurrentPlc != HongDianDTUS.S200Smart)
            {
                // need read one time
        ...
        private HongDianDTUS CurrentPlc = HongDianDTUS.S1200;
        ...
        /// <param name="length">读取数据长度 -> Read Data length</param>
        /// <returns>包含结果对象的报文 -> Message containing the result object</returns>
        public static OperateResult<byte[]> BuildDDPReadCommand(OperateResult<byte, int, ushort>[] address, ushort[] length)
        {
            if (address == null) throw new NullReferenceException("address");
            if (length == null) throw new NullReferenceException("count");
            if (address.Length != length.Length) throw new Exception(StringResources.Language.TwoParametersLengthIsNotSame);
