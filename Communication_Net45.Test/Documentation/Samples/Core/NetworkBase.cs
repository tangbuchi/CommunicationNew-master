...
namespace Communication_Net45.Test.Documentation.Samples.Core
{
    public class NetworkBaseExample
    {
        ...
        private void SimplifyServer_ReceiveStringEvent( Communication.Core.Net.AppSession session, Communication.NetHandle handle, string data )
        {
            // 示例情况，接收到数据后返回消息
        ...
        public void LogNetExample( )
        {
            ...
            // 举例实现日志文件为单日志文件
            melsec.LogNet = new Communication.LogNet.LogNetSingle( "D://123.txt" );
            ...
            simplifyServer.LogNet = new Communication.LogNet.LogNetSingle( "D://log.txt" );
            simplifyServer.ReceiveStringEvent += ( Communication.Core.Net.AppSession session, Communication.NetHandle handle, string data ) =>
            {
                simplifyServer.SendMessage( session, handle, "Back:" + data );
            };
