...
namespace Communication_Net45.Test.Profinet.AllenBradley
{
    [TestClass]
    public class AllenBradleyHelperTest
    {
        ...
        public void PackRequsetReadTest( )
        {
            ...
            if (!Communication.BasicFramework.SoftBasic.IsTwoBytesEquel( buffer, corrent ))
            {
                Assert.Fail( "指令失败：" + Communication.BasicFramework.SoftBasic.ByteToHexString( buffer ) );
            }
        }
