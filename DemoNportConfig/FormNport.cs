...
namespace DemoNportConfig
{
    public partial class FormNport : Form
    {
        public FormNport()
        {
            InitializeComponent();
        ...
        private void ComboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (busTcpClient != null)
            {
                ...
                {
                    case 0: busTcpClient.DataFormat = Communication.Core.DataFormat.ABCD; break;
                    case 1: busTcpClient.DataFormat = Communication.Core.DataFormat.BADC; break;
                    case 2: busTcpClient.DataFormat = Communication.Core.DataFormat.CDAB; break;
                    case 3: busTcpClient.DataFormat = Communication.Core.DataFormat.DCBA; break;
                    default: break;
                }
            }
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            busTcpClient?.ConnectClose();// 连接关闭
            ...
            try
            {
                OperateResult connect = busTcpClient.ConnectServer(); // 连接服务
                ...
                {
                    MessageBox.Show(Communication.StringResources.Language.ConnectedFailed);
                }
            }
            catch (Exception ex)
            {
        ...
        {
            OperateResult<byte[]> read = busTcpClient.ReadFromCoreServer(Communication.BasicFramework.SoftBasic.HexStringToBytes(textBox13.Text));
            if (read.IsSuccess)
            {
        ...
        private void Test2()
        {
            ...
            Communication.Core.IByteTransform ByteTransform = new Communication.Core.ReverseWordTransform();
        }
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            try
            {
                OperateResult<byte[]> read = busTcpClient.Read("0", ushort.Parse("20"));
                if (read.IsSuccess)
                {
                    textBox10.Text = "Result：" + Communication.BasicFramework.SoftBasic.ByteToHexString(read.Content);
                }
                else
                {
                    MessageBox.Show("Read Failed：" + read.ToMessageShowString());
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            try
            {
                OperateResult<byte[]> read = busTcpClient.Read("20", ushort.Parse("160"));
                if (read.IsSuccess)
                {
                    textBox10.Text = "Result：" + Communication.BasicFramework.SoftBasic.ByteToHexString(read.Content);
                }
                ...
                ushort value1 = busTcpClient.ByteTransform.TransUInt16(read.Content, 0);
                ushort value2 = busTcpClient.ByteTransform.TransUInt16(read.Content, 2);
                ushort value3 = busTcpClient.ByteTransform.TransUInt16(read.Content, 4);
                ushort value4 = busTcpClient.ByteTransform.TransUInt16(read.Content, 6);
                string str = $"{value1}.{value2}.{value3}.{value4}";
                if (System.Net.IPAddress.TryParse(str, out System.Net.IPAddress address))
                {
                    textBox12.Text = str;
                } 
